...
题目里的六条需求，每一条配一条能演示它的命令，以及守住它的测试。任何一行你都可以自己跑；这里没有一行需要联网。
...
下面这些是这个实现会招来的问题，以及代码支撑得住的回答。

**"我怎么知道你的数字守卫真的在工作，而不是因为从来没出过错才通过？"**
`art demo --fault N` 会在 mock 模型的输出里精确篡改 `N` 个数字，而守卫必须报告全部 `N` 个。`tests/test_demo.py` 在内置页面上对 `N = 1..6`、在录制 fixture 上对 `1..9` 断言了这一点，而 `test_e2e.py` 交叉校验*实际应用的*篡改数等于请求数——所以这个判定不可能被一个静默什么都没做的注入器满足。另有一个测试断言这个判定*可以*失败，所以这个检查不是空的。

**"为什么要费劲搞 SDO？直接传 HTML 不行吗？"**
因为可审计性需要**地址**。一条发现必须能指向页面上的一个矩形：`(page, block, row, col)`。HTML 能渲染一张表，但它没法告诉你*哪个图*在*哪个单元格*里对不上，而且它对空洞或合并冲突没有表示。SDO 才是让守卫、风险策略和复核者读到**同一个事实**的东西。

**"当 VLM 的框是错的怎么办？"**
容差按最窄单元格缩放，所以抖动被吸收；几何覆盖提示；而无法调和的输入产出一张降级的表加一条警告，而不是一个自信的猜测。`test_table_builder.py` 里有一个抖动恢复测试和一个"几何打败坏提示"测试。

**"两个数字对不上——哪个是对的？"**
谁都不假设为对；**源文件是权威**。守卫报告一条不匹配，给出源值、目标值和机制（单位变化、数字滑位、单位注丢失），而风险分把这个块路由给人。流水线从不悄悄选一个赢家。

**"这要怎么扩展到一份 300 页的报告？"**
切块是章节驱动的，所以工作按块并行，而数字守卫和风险打分是线性遍历。抗拒并行的部分是那些必须文档全局的——术语表构建和数字比对——它们跑一次，跑在便宜的数据上，而不是每块都跑。

**"你每周重跑一份文档。怎么防止复核者永远看到同样的条目？"**
`item_id` 由 `chunk_id` 推导，所以重跑刷新的是已有条目，而不是追加一个新的——这正是让队列可 diff 的前提。一个决定只在证据不变时才被带过来；如果输出动了，条目重开为 pending，先前的决定被留作审计备注。测试套件覆盖两半，因为**第一版这里做错了**：随机 id 意味着每次重跑都把队列翻倍，并让记在它上面的决定成为孤儿。

**"如果给你更多时间，你会做什么不同的事？"**
三件事，按对产品的改善程度排序：(1) 把数字守卫的单位模型做成感知币种的，而不是只感知尺度，因为跨币种重述是目前唯一会被它过度报告的不匹配；(2) 把术语批准持久化进跟翻译同一个复核队列，让术语决定能跨*文档*累积，而不只是在一份文档内；(3) 加一个投射步骤，把译后的 SDO 渲染回 PDF——那是从"正确"到"可交付"的最后一公里。
